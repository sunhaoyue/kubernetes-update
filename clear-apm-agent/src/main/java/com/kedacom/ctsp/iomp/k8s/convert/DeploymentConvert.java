 ...
 */
@Slf4j
public class DeploymentConvert implements K8sConvert<Deployment, DeploymentEditDto> {


    @Override
    public DeploymentEditDto convert(Deployment from) {
        DeploymentEditDto result = new DeploymentEditDto();
        result.setReplicas(from.getSpec().getReplicas());
        result.setStrategy(from.getSpec().getStrategy());
...
/*        result.setBgp();
        result.setName();
        result.setNamespace();
        result.setProjectImage();
        result.setBaseImage();
        result.setReleaseInfo();*/
     ...
     * @return
     */
    private Container getMainContainer(Deployment from) {
    ...
    private Container geInitContainer(Deployment from) {
        ...
        for (Container container : containers) {
            // 不是apm-agent的，才返回；
            if (!StringUtils.contains(container.getName(), "apm")) {
                return container;
            }
        }
        return null;
    ...
    @Override
    public Deployment merge(DeploymentEditDto source, Deployment target) {
        if (source == null || target == null) {
            return target;
        }
        log.info("last deployment:" + JSONObject.toJSONString(target));
        Deployment newDeployment = new DeploymentBuild(target)
                .setIsPatch(source.getIsPatch())
                .setReplicas(source.getReplicas())
                .setRollingUpdate(source.getStrategy())
    ...
    class DeploymentBuild {
        ...
        private DeploymentBuild(Deployment deployment) {
        ...
        public DeploymentBuild setIsPatch(Boolean isPatch) {
            if (null == isPatch) {
        ...
        public DeploymentBuild setReplicas(Integer replicas) {
        ...
        public DeploymentBuild setRollingUpdate(DeploymentStrategy strategy) {
            if (null == strategy && this.isPatch) {
                return this;
            }
            deployment.getSpec().setStrategy(strategy);
            return this;
        }

        public DeploymentBuild setLimitGpu(Long gpu) {
        ...
        public DeploymentBuild setLimitCpu(String cpu) {
        ...
        public DeploymentBuild setLimitMemory(String memory) {
        ...
        public DeploymentBuild setRequestMemory(String memory) {
            if (null == memory && this.isPatch) {
                return this;
            }
        ...
        public DeploymentBuild setPorts(List<ContainerPort> ports) {
        ...
        public DeploymentBuild setLivenessProbe(Probe probe) {
        ...
        public DeploymentBuild setReadinessProbe(Probe probe) {
            if (null == probe && this.isPatch) {
                return this;
            }
        ...
        public DeploymentBuild setEnv(Map<String, EnvVar> env) {
        ...
        public DeploymentBuild setImage(String projectImage) {
        ...
        public DeploymentBuild setBaseImage(String baseImage) {
        ...
        public DeploymentBuild setBgp(String bgp) {
        ...
        public DeploymentBuild setIpv4pools(String ipv4pools) {
        ...
        public DeploymentBuild setAnnotations(Map<String, String> ann) {
        ...
        public DeploymentBuild removePvc() {
        ...
        public DeploymentBuild setVolumeMounts(List<VolumeDto> volumeDtos) {
        ...
        private DeploymentBuild removeCaAndKubectlAndTimezoneMount() {
            List<Volume> volumes = deployment.getSpec().getTemplate().getSpec().getVolumes();
            if (CollectionUtils.isEmpty(volumes)) {
                return this;
            }
        ...
        public DeploymentBuild setAffinity(Affinity affinity) {
        ...
        public DeploymentBuild setLogContainer(String logStatus, Container logNewContainer, String clientId) {
        ...
        public DeploymentBuild setApmContainer(String apmStatus, Container newApmContainer) {
            ...
            //走到这  说明apm没开 将java_opts里的 -javaagent:/apm-agent/skywalking-agent.jar 拿掉
            Container mainContainer = getMainContainer(deployment);
            for (EnvVar envVar : mainContainer.getEnv()) {
                if (!StringUtils.equals(envVar.getName(), "JAVA_OPTS")) {
                    continue;
                }
                String javaOpts = StringUtils.replace(envVar.getValue(), "-javaagent:/apm-agent/skywalking-agent.jar", StringUtils.EMPTY);
        ...
        public DeploymentBuild setLabels(Map<String, String> labels) {
        ...
        public DeploymentBuild setServiceAccount(String serviceAccount) {
        ...
        public DeploymentBuild setTolerations(List<Toleration> tolerations) {
        ...
        public DeploymentBuild setPromContainer(Integer enableCustomMetric, Container newPromContainer) {
        ...
        public DeploymentBuild setHostNetWork(boolean isHostNetWork) {
            if (isHostNetWork) {
        ...
        public DeploymentBuild setPriorityClass(String priorityClassName) {
